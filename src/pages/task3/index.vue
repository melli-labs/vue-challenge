...
const allNotes: Note[] = ref([]);

const isFetching = ref(false);
const input = ref('');
const { results } = useFuse(input, allNotes, {
...
const getNotes = () => {
    ...
    fetch('https://emilia-vue-challenge.deta.dev/notes')
...
<template>
    <div class="grid gap-6">
        <div class="flex gap-4 flex-col sm:flex-row">
            ...
            >
                <div
                    class="h-6 text-primary-700 w-6 i-heroicons-outline:search"
                />
                <input
                    v-model="input"
