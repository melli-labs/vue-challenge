...
const mockData: Note[] = [
  {
    ...
    body: 'There are some notes which are readonlyآ ًں”’ï¸ڈ. These notes cannot be edited or deleted. But for the other notes we should be able to use the dropdown to editآ âœچï¸ڈ or deleteآ â‌Œ them.',
    author: 'Emilia',
    key: 'f24jv9ss',
    readonly: true,
  ...
  {
    title: 'Tip: useFetch from VueUse ًں§©',
    body: 'Consider using the useFetch utility from the VueUse package for this task. It provides a convenient and reactive wrapper for the Fetch API. But of course you are free to install any other fetching library ًں“¥ï¸ڈ or just use the Fetch API directly.',
    author: 'Felix',
    key: 'j3hhw92j',
    readonly: true,
...
const dbnotes: Ref<Note[]> = ref([]);
const getdatafromdb = async () => {
  const { data } = await useFetch(`https://emilia-vue-challenge.deta.dev/notes`)
    .get()
    .json();
  dbnotes.value = data.value;
};
onMounted(() => {
  getdatafromdb();
});
...
const input = ref("");
const { results } = useFuse(input, dbnotes, {
