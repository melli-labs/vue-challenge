...
const getNotes = async() => {
  const { isFetching, data } = await useFetch(`${baseUrl}/notes`).get().json()
...
<template>
  <div class="grid gap-6">
    ...
    <div v-if="isLoading" class="min-h-xs grid place-items-center">
    ...
    <div v-else-if="results.length > 0" class="grid gap-4 md:min-w-screen-sm lg:grid-cols-2">
      <Note v-for="(result, index) in results" :key="index" :note="result.item" @get-notes="getNotes" />
    </div>
