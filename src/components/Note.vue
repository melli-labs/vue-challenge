...
const deleteNote = () => {
    fetch(`https://emilia-vue-challenge.deta.dev/notes/${props.note.key}`, {
        method: 'DELETE',
    })
        .then((response) => {
            if (response.ok) {
                ...
                throw new Error('Note could not deleted');
            }
        })
        .catch((e) => {
            alert('Note could not deleted');
            console.log(e);
        });
};
const updateNote = () => {
    ...
    fetch(
        ...
        .then((response) => {
            if (response.ok) {
                alert('Note updated successful');
                console.log(response);
                isUpdating.value = false;
                window.location.reload();
            } else {
                throw new Error('Note could not updated');
            }
        })
        .catch((e) => {
            alert('Note could not updated');
            console.log(e);
            isUpdating.value = false;
        });
};
            ...
            <div v-else class="ml-auto relative inline-block ml-2">
                ...
                >
                    ...
                    >
                        ...
                        >
                            <span
                                class="i-heroicons-outline:pencil-alt w-6 h-6 opacity-65"
                            />
                            Edit
                        </button>
                        <button
                        ...
                        >
                            <span
                                class="i-heroicons-outline:x w-6 h-6 opacity-65"
                            />
                            Delete
                        </button>
                    </div>
