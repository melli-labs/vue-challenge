...
const noteid = ref(props.note.key);

const deleteNote = () => {
  emiliaApi
    .delete(`https://emilia-vue-challenge.deta.dev/notes/${noteid.value}`)
    .then(() => {
      alert("delete done");
    })
    .catch(() => {});
...
const updateNote = () => {
  ...
  emiliaApi
    .put(`https://emilia-vue-challenge.deta.dev/notes/${noteid.value}`, {
    })
    .then(() => {
      alert("edite done");
    })
  ...
  >
    ...
    >
      ...
      <div>
        ...
        <span v-if="note.updatedAt !== note.createdAt">آ· edited</span>
      </div>
      <div
        v-if="note.readonly"
        class="
